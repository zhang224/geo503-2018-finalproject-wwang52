...
sf_population%>%ggplot()+
  geom_sf(aes(geometry=geometry,fill=-log(POP2010,base=100)))+
  scale_fill_viridis_c(name="-log100(population)")+
...
ggplot()+
  geom_sf(data=sf_population,aes(geometry=geometry,fill=-log(POP2010,base=100)))+
  scale_fill_viridis_c(name="-log100(population)")+
  geom_sf(data=sf_schools,aes(colour="a"))+
  scale_colour_manual(values=c("a"="orange")) +
  theme(legend.position = 'bottom')
```
(still on working~)
